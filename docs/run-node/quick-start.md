...
## IP Address Ranges to Block with Firewall on VDS

Hosting providers for virtul dedicated servers (VDS) typically provide a public IP address while expecting the software running on your server to address other communication endpoints also via public IP addresses. Any attempts to communicate with private address ranges are typically interpreted by the hosting provider as a network attack with the warnings being sent to the server operator and, if not corrected quickly, with the server being suspended.
...
To prevent connection attempts to the private IP ranges on a VDS, the following firewalls rules can be added on Ubuntu Linux with `ufw` utility: 
